...
public class AngelinaBeanPostProcessor implements BeanPostProcessor {
    ...
    /**
     * 全量扫描所有Bean，查找每一个被@Angelina注解的方法。
     * 1.检查方法参数是否符合要求
     * 2.检查keyWords是否有重复
     * 3.加载所有参数到到AngelinaContainer中
     */
    @Override
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        for (Method method: methods) {
            ...
            if (angelinaGroup != null) {
                checkOutAngelinaGroup(angelinaGroup, method, clazz);
            }

            AngelinaFriend angelinaFriend = method.getAnnotation(AngelinaFriend.class);
            if (angelinaFriend != null) {
                checkOutAngelinaFriend(angelinaFriend, method, clazz);
            }

            AngelinaEvent angelinaEvent = method.getAnnotation(AngelinaEvent.class);
            if (angelinaEvent != null) {
                checkOutAngelinaEvent(angelinaEvent, method, clazz);
            }
        }
        return BeanPostProcessor.super.postProcessAfterInitialization(bean, beanName);
    }

    private void checkOutAngelinaGroup(AngelinaGroup annotation, Method method, Class<?> clazz) {
        //获取文字关键字
        String[] groupKeyWords = annotation.keyWords();
        ...
        //确认完全符合要求后，将关键字和方法添加至全局变量groupMap中
        AngelinaContainer.groupMap.put(name, method);

        String[] dHashList = annotation.dHash();
    ...
    private void checkOutAngelinaFriend(AngelinaFriend annotation, Method method, Class<?> clazz) {
        String[] friendKeyWords = annotation.keyWords();
    ...
    private void checkOutAngelinaEvent(AngelinaEvent annotation, Method method, Class<?> clazz) {
        EventEnum eventEnum = annotation.event();
        //判断关键字是否重复
